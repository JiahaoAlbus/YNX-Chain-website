...
| 11 | Developer/AI Build | 公开 Web、macOS/Windows 预览包 | Testnet 功能已恢复，生产签名仍缺 | Web 已真实连接 6423 链、solc 0.8.24 编译、YNX 托管 qwen3:4b AI（2 并发/32 排队）和 xAI/OpenAI 自带 Key；macOS 包已验证装库、测试、二次启动依赖持久化，Windows run 31261305218 已验证编译、解压和冷启动；两包仍未生产签名 |
...
- Developer 已按用户指定顺序完成 Testnet 可用性修复；剩余边界为中央 Wallet 接受、生产签名/公证和商店发布，不能伪报完成。
