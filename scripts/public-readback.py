...
def identity(url):
    ...
    if not metadata["contentType"].lower().startswith("application/json"):
        raise ValueError("IDENTITY_CONTENT_TYPE")
    ...
    if parsed.get("chainId") != 6423 or parsed.get("schemaVersion") != "ynx.website.build-identity.v1":
        raise ValueError("IDENTITY_SCHEMA_MISMATCH")
    return {**metadata, "sourceCommit": parsed["sourceCommit"], "sourceTree": parsed["sourceTree"], "release": parsed.get("release")}


def html(url):
    body, metadata = get(url, 2_000_000)
    if not metadata["contentType"].lower().startswith("text/html"):
        raise ValueError("HTML_CONTENT_TYPE")
    text = body.decode("utf-8")
    if COPY_CHUNK not in text or f"/assets/{ENTRY}" not in text:
        raise ValueError("HTML_NEW_ASSET_MISMATCH")
    if not re.search(r'<script[^>]+src="/assets/' + re.escape(ENTRY) + r'"', text):
        raise ValueError("HTML_ENTRY_SCRIPT_MISSING")
    return metadata
...
def javascript(url):
    ...
    if "javascript" not in metadata["contentType"].lower():
        raise ValueError("SCRIPT_CONTENT_TYPE")
    if b"0.1.4" not in body:
        raise ValueError("SCRIPT_CHROME_014_MISMATCH")
...
def zipfile(url):
    ...
    if metadata["bytes"] != CHROME_BYTES or metadata["sha256"] != CHROME_SHA256:
        raise ValueError("CHROME_ZIP_MISMATCH")
    if not body.startswith(b"PK\x03\x04"):
        raise ValueError("CHROME_ZIP_FORMAT")
    return metadata
