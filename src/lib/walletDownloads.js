...
export function walletDownloadState(platform, item, registryAllowsDownloads = true) {
  ...
  const browserPreview = ["wallet-static-20260906-r4c-browser", "wallet-web-testnet-preview-0.1.1-c93e16be8", "wallet-web-testnet-preview-0.1.2-767a05d56", "wallet-web-testnet-preview-0.1.3-31f3ef16d"].includes(item?.releaseBatch);
  const chromeEdge012 = platform === "chromeEdge" && item?.releaseBatch === "wallet-web-testnet-preview-0.1.2-767a05d56";
  const chromeEdge013 = platform === "chromeEdge" && item?.releaseBatch === "wallet-web-testnet-preview-0.1.3-31f3ef16d";
  ...
  return {
    ...
    limitationKey: desktop068 ? "desktop068Boundary" : androidCurrent ? "android27Boundary" : permissionHold ? "firefoxPermissionHold" : legacyBlocked ? "macLegacyBlocked" : available ? item.historicalPreview ? "historicalBoundary" : macosPreview ? "macosPreviewBoundary" : androidPreview || androidUniversal ? "androidPreviewBoundary" : desktopPreview ? "desktopPreviewBoundary" : chromeEdge013 ? "browser013Boundary" : chromeEdge012 ? "browser012Boundary" : browserPreview ? "browserPreviewBoundary" : "previewUnverified" : "releasePending",
    installProofKey: desktop068 ? "desktop068Proof" : androidCurrent ? "android27Proof" : androidUniversal ? "androidUniversalProof" : macosPreview ? "macosLimitedProof" : androidPreview ? "androidLimitedProof" : desktopPreview ? item.installation === "appimage" ? "appImageNotInstalled" : "limitedCiLaunch" : chromeEdge013 ? "browser013Proof" : chromeEdge012 ? "browser012Proof" : browserPreview ? platform === "pwa" ? "pwaArchiveOnly" : "manualExtension" : null,
