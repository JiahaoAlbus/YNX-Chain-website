...
export function PortalPage({ path }) {
  ...
  return <main className="portalPage" id="main-content">
    ...
    <section className="portalStatus" aria-label={t("currentNetworkVerification")}><div><span className={`statusPill ${network.ok === true ? "verified" : network.degraded ? "degraded" : "unavailable"}`}>{network.ok === true ? <ShieldCheck /> : <FileWarning />}{health}</span><h2>{t("current6423Verification")}</h2><p>{network.checkedAt ? `${t("checked")} ${new Date(network.checkedAt).toLocaleString(locale)}` : t("fetchingPublicServices")}</p></div><div className="portalLiveValues"><span data-chain-verified={network.chainVerified === true}><small>{t("rpcHeight")} · {network.chainVerified === true ? t("verified") : t("unavailable")}</small><strong>{network.status?.height ?? t("unavailable")}</strong><small>{Number.isFinite(network.observations?.rpcCollectionMs) ? `${network.observations.rpcCollectionMs} ms` : t("unavailable")}</small></span><span><small>{t("indexerHeight")}</small><strong>{network.explorer?.indexedHeight ?? t("unavailable")}</strong><small>{Number.isSafeInteger(network.indexerLagBlocks) ? `Δ ${network.indexerLagBlocks}` : t("unavailable")}</small></span><span data-indexer-verified={network.indexerVerified === true}><small>{t("indexerState")}</small><strong>{network.indexerVerified === true ? t("aligned") : t("unavailable")}</strong></span></div></section>
