...
export function PortalPage({ path }) {
  ...
  const initialRpcRead = collection(observations.initialRpcCollectionMs, observations.initialRpcReadState);
  ...
  const growth = observations.progressionState === "observed" ? `${metrics.observed} (${observations.progressionFromHeight} → ${observations.progressionToHeight})` : observations.progressionState === "not_observed" ? metrics.notObserved : metrics.unverified;
  const config = [
