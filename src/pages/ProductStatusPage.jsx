    ...
    {!overview ? <SectionPage product={product} contract={contract} sectionId={sectionId} guide={guide} copy={copy} locale={locale} /> : <>
      <section className="productStatusLayout">
        <div className="productStatusMain">
          ...
          <div className="productStatusSection"><div className="sectionHeader compact"><div><p className="sectionEyebrow">Platforms</p><h2>Install and access</h2></div></div><ul className="productSurfaceList">{(product.key === "wallet" ? ["web", ...WALLET_DOWNLOAD_PLATFORMS] : platformOrder).map((platform) => <Surface key={platform} platform={platform} item={product.downloads?.[platform]} registryAllowsDownloads={contract.downloadHostedVerified} productKey={product.key} />)}</ul>{product.key === "wallet" && <WalletDownloadHistory locale={locale} />}</div>
        </div>
