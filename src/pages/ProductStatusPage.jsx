...
function Surface({ platform, item, registryAllowsDownloads, productKey }) {
  ...
  return <li className={`productSurface ${item?.status || PRODUCT_STATUS.NOT_READY}`}><span><strong>{productKey === "wallet" ? walletDownloadLabel(platform, copy) : DOWNLOAD_LABELS[platform]}</strong><small>{walletFile?.safetyHold ? <WalletDownloadSafetyNotice locale={locale} hold={walletFile.safetyHold} /> : walletFile ? copy[walletFile.limitationKey] : item?.note || "No verified release evidence."}</small>{walletFile?.signingKey && <small>{copy[walletFile.signingKey]}</small>}{walletFile?.installProofKey && <small>{copy[walletFile.installProofKey]}</small>}{walletFile?.fallbackHref && <small>{copy.fallbackManual}</small>}</span>{canOpen ? <span className="productSurfaceActions"><a href={item.href} download={walletFile?.filename} data-download-source={walletFile ? "official" : undefined} rel={item.external ? "noopener" : undefined}>{walletFile ? copy.download : "Download"} <ArrowUpRight size={14} /></a>{walletFile?.fallbackHref && <a href={walletFile.fallbackHref} download={walletFile.filename} data-download-source="github-fallback" rel="noopener">{copy.githubFallback}<ArrowUpRight size={14}/></a>}</span> : <em>{walletFile?.safetyHold ? getWalletSafetyCopy(locale).pausedLabel : walletFile ? copy.unavailableYet : item?.href ? "Registry verification required" : state.text}</em>}</li>;
}
...
export function ProductStatusPage({ product, sectionId = "overview", article, artifact }) {
  ...
  const professionalHost = { exchange: "https://exchange.ynxweb4.com/", quant: "https://quant.ynxweb4.com/" }[product.key];
  const professionalTerminal = professionalHost && contract.publicEntry.status === "available" && contract.publicEntry.href === professionalHost;
    ...
    {!overview ? <SectionPage product={product} contract={contract} sectionId={sectionId} guide={guide} copy={copy} locale={locale} /> : <>
      <section className="productStatusLayout">
        <div className="productStatusMain">
          ...
          <div className="productStatusSection"><div className="sectionHeader compact"><div><p className="sectionEyebrow">Platforms</p><h2>Install and access</h2></div></div>{product.key === "wallet" && <WalletDownloadInventoryNotice locale={locale} />}<ul className="productSurfaceList">{(product.key === "wallet" ? ["web", ...WALLET_DOWNLOAD_PLATFORMS] : platformOrder).map((platform) => <Surface key={platform} platform={platform} item={product.downloads?.[platform]} registryAllowsDownloads={contract.downloadHostedVerified} productKey={product.key} />)}</ul>{product.key === "wallet" && <WalletDownloadHistory locale={locale} />}</div>
        </div>
        <aside className="productStatusAside"><div><GitBranch size={18} /><span><small>Registry source commit</small><code>{contract.registryCommit || "No registry record"}</code></span></div><p>{product.release?.statusNote || "No product release record has been accepted."}</p>{professionalTerminal && <a className="button primary" href={professionalHost} rel="noopener" data-professional-entry={product.key}>{copy.openInBrowser} · {product.name} <ArrowUpRight size={16} /></a>}<a className={professionalTerminal ? "button secondary" : "button primary"} href={contract.sections.find((item) => item.id === "docs-api").href}>Docs and API <ArrowUpRight size={16} /></a><a className="button secondary" href={contract.sections.find((item) => item.id === "open-download").href}>Open or download <ArrowUpRight size={16} /></a><a className="button secondary" href={contract.sections.find((item) => item.id === "releases").href}>Release evidence <ArrowUpRight size={16} /></a><a className="textLink" href="/dapp/download"><Download size={16} /> Download center</a></aside>
      </section>
