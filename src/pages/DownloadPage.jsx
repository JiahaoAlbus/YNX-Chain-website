  ...
  return (
    <main className="downloadPage downloadHub" dir={locale === "ar" ? "rtl" : undefined}>
      ...
      {selected ? <section className="downloadHubFeature" aria-label={`${selected.name} ${experience.choose}`} data-selected-product={selected.key}>
        ...
        <div className="downloadHubFeatureChoices"><strong>{experience.choose}</strong>{featurePackages.length ? <div className="downloadHubPackageLinks">{featurePackages.map(([key, item]) => <a key={key} href={item.href} rel={item.external ? "noopener" : undefined} download={item.artifactPath || undefined} aria-label={`${selected.name} ${DOWNLOAD_LABELS[key] || key} ${copy.download}`}><Download size={15}/>{selected.key === "wallet" ? walletDownloadLabel(key, { ...(PRODUCT_UI_COPY[locale] || PRODUCT_UI_COPY.en), ...(WALLET_DOWNLOAD_COPY[locale] || WALLET_DOWNLOAD_COPY.en) }) : DOWNLOAD_LABELS[key] || key}</a>)}</div> : <p>{publicWeb ? experience.platformUnavailable : experience.noPublic}</p>}</div>
        <div className="downloadHubFeatureActions"><a className="downloadHubPrimary" href={productSectionRoute(selected.route, "open-download")}>{experience.versions}<ArrowUpRight size={17}/></a>{publicWeb && <a className="downloadHubSecondary" href={publicWeb} rel="noopener">{experience.openWeb}<ArrowUpRight size={16}/></a>}</div>
