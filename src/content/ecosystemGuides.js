export const ECOSYSTEM_GUIDES = {
  ...
  browser: {
    purpose: "Navigate YNX and external web surfaces with platform-native engines while keeping site content, private browsing data, device permissions and Wallet authority in separate trust boundaries.",
    workflow: ["Download the arm64 macOS Testnet Preview and verify its SHA-256 before opening it.", "Remove or unregister older YNX Browser previews so the ynxbrowser callback cannot resolve to an obsolete copy.", "Open a destination and continuously inspect the exact origin, TLS/navigation state and whether the tab is normal or private.", "Grant camera, files, clipboard and other capabilities separately for the current origin; private permissions never persist.", "When a DApp requests a transaction, Browser creates a bounded signed request and hands the exact review to Wallet; Browser never signs or creates a local Product Session.", "Review callback failure codes, history/download records and recovery state; delete selected normal data without exposing private traces."],
    rules: ["Web content cannot request or receive a seed phrase, recovery key or Wallet private key.", "An origin, callback route, nonce, expiry, chain ID, product client, bundle ID, device key or requested scope mismatch fails closed.", "Callbacks are single use and replay-safe; malformed, duplicate, oversized, expired or unknown fields are rejected before pending state is consumed where applicable.", "Normal and private tab identity cannot be changed by page content. Private tabs, downloads and permissions must not be written into persistent normal-state records.", "Camera, files, clipboard, location, notifications, downloads and Wallet review are separate permissions, not one blanket approval.", "The current public artifact is an arm64 Testnet Preview. It is ad-hoc signed, rejected by Gatekeeper, not notarized, not Developer ID signed and not an App Store release.", "A hosted ZIP proves only the registered bytes and tests. It does not prove production signing, cross-host reproducibility, Windows packaging or a positive shared-Testnet Wallet callback."],
  },
  search: {
