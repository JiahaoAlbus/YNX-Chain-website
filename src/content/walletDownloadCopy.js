...
for (const [locale, copy] of Object.entries(WALLET_DOWNLOAD_COPY)) {
 ...
 copy.android27Boundary = locale === "zh-CN" ? "1.0.21 测试网预览 APK，仅 Android Debug 测试签名、未上架。GitHub 发布者可替换文件；网站不在下载时重算 SHA-256，请核对摘要。" : locale === "zh-TW" ? "1.0.21 測試網預覽 APK，僅 Android Debug 測試簽名、未上架。GitHub 發布者可替換檔案；網站不在下載時重算 SHA-256，請核對摘要。" : "1.0.21 Testnet Preview APK; Android Debug test-signed, not store released. GitHub assets are publisher-replaceable; verify SHA-256 after download.";
 copy.android27Proof = locale === "zh-CN" ? "已验证精确发布 APK 在 API 36 模拟器上从 code26 升至 code27，保留并解锁一次性生物识别账户。已充值余额、待发交易恢复、真机、Finance 与 Relay 端到端仍未验证。" : locale === "zh-TW" ? "已驗證精確發布 APK 在 API 36 模擬器上從 code26 升至 code27，保留並解鎖一次性生物辨識帳戶。已充值餘額、待發交易恢復、實機、Finance 與 Relay 端到端仍未驗證。" : "Exact published APK upgraded code26 to 27 on an API 36 emulator and retained/unlocked a disposable biometric account. Funded balance, pending outbox recovery, physical device, Finance and Relay E2E remain unverified.";
}
