...
for (const [locale, copy] of Object.entries(WALLET_DOWNLOAD_COPY)) {
 copy.android23Boundary = locale === "zh-CN" ? "1.0.17 测试网预览版，仅测试签名、未上架。真实 Relay、真机转账及已安装 Finance 流程未验证；缺 Reown ID 禁止配对。releaseImmutable=false：发布者可替换文件；网站下载时不重算 SHA-256，请自行核对摘要。" : locale === "zh-TW" ? "1.0.17 測試網預覽版，僅測試簽名、未上架。真實 Relay、實機轉帳及已安裝 Finance 流程未驗證；缺 Reown ID 禁止配對。releaseImmutable=false：發布者可替換檔案；網站下載時不重算 SHA-256，請自行核對摘要。" : "1.0.17 Testnet Preview; test signed, not store released. Real Relay, physical transfers and installed Finance E2E unverified; no Reown ID means no pairing. releaseImmutable=false: publisher can replace assets. Website does not recompute SHA-256 during download; verify the digest yourself.";
 copy.android23Proof = locale === "zh-CN" ? "发布证据：API 36 模拟器从 versionCode 22 原位升级至 23，保留安装时间；冷启动 220ms，深链无崩溃。56/56 恢复测试不代表真实链上转账。" : locale === "zh-TW" ? "發布證據：API 36 模擬器從 versionCode 22 原位升級至 23，保留安裝時間；冷啟動 220ms，深鏈無崩潰。56/56 恢復測試不代表真實鏈上轉帳。" : "Owner evidence: API 36 emulator upgraded versionCode 22 to 23, preserving install time; cold launch 220ms, deep links without crash. 56/56 recovery tests do not prove live-chain transfer.";
}
