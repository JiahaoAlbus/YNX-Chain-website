...
for (const [locale, copy] of Object.entries(WALLET_DOWNLOAD_COPY)) {
 copy.android24Boundary = locale === "zh-CN" ? "1.0.18 测试网预览版，仅测试签名、未上架。真实 Relay、真机转账及已安装 Finance 流程未验证；缺 Reown ID 禁止配对。releaseImmutable=false：发布者可替换文件；网站下载时不重算 SHA-256，请自行核对摘要。" : locale === "zh-TW" ? "1.0.18 測試網預覽版，僅測試簽名、未上架。真實 Relay、實機轉帳及已安裝 Finance 流程未驗證；缺 Reown ID 禁止配對。releaseImmutable=false：發布者可替換檔案；網站下載時不重算 SHA-256，請自行核對摘要。" : "1.0.18 Testnet Preview; test signed, not store released. Real Relay, physical transfers and installed Finance E2E unverified; no Reown ID means no pairing. releaseImmutable=false: publisher can replace assets. Website does not recompute SHA-256 during download; verify the digest yourself.";
 copy.android24Proof = locale === "zh-CN" ? "发布证据：APK/AAB 摘要及固定路径两次构建一致；Android 安装、冷启动与真机均未验证，不沿用旧版模拟器证据。" : locale === "zh-TW" ? "發布證據：APK/AAB 摘要及固定路徑兩次建置一致；Android 安裝、冷啟動與實機均未驗證，不沿用舊版模擬器證據。" : "Owner evidence: APK/AAB digests and two fixed-path builds matched. Android install, cold launch and physical devices remain NOT_VERIFIED; older emulator evidence is not inherited.";
}
