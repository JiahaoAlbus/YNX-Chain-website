...
for (const [locale, copy] of Object.entries(WALLET_DOWNLOAD_COPY)) {
 copy.android25Boundary = locale === "zh-CN" ? "1.0.19 测试网预览版，仅测试签名、未上架。真实 Relay、真机转账及已安装 Finance 流程未验证；缺 Reown ID 禁止配对。releaseImmutable=false：发布者可替换文件；网站下载时不重算 SHA-256，请自行核对摘要。" : locale === "zh-TW" ? "1.0.19 測試網預覽版，僅測試簽名、未上架。真實 Relay、實機轉帳及已安裝 Finance 流程未驗證；缺 Reown ID 禁止配對。releaseImmutable=false：發布者可替換檔案；網站下載時不重算 SHA-256，請自行核對摘要。" : "1.0.19 Testnet Preview; test signed, not store released. Real Relay, physical transfers and installed Finance E2E unverified; no Reown ID means no pairing. releaseImmutable=false: publisher can replace assets. Website does not recompute SHA-256 during download; verify the digest yourself.";
 copy.android25Proof = locale === "zh-CN" ? "1.0.19 已验证全新 API 36 模拟器安装、冷启动、锁定恢复及强生物识别。只读结果：链上无账户记录；资金、真机、转账与 Finance/Relay 未验证。" : locale === "zh-TW" ? "1.0.19 已驗證全新 API 36 模擬器安裝、冷啟動、鎖定恢復及強生物辨識。唯讀結果：鏈上無帳戶紀錄；資金、實機、轉帳與 Finance/Relay 未驗證。" : "1.0.19 fresh API 36 emulator install, cold launch, locked recovery and biometrics passed. Read-only: no on-chain account record. Funded state, physical devices, transfers, Finance and Relay unverified.";
}
