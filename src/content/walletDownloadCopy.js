...
const DESKTOP_0610_COPY = {
  "zh-CN": ["Windows x64 0.6.10 无签名测试预览；系统重启、用户设备、SmartScreen/Defender、已失败档案恢复和真实交易未验证。", "原始 0.6.10 安装包在 Windows CI 中通过从 0.6.8 与 0.6.9 升级，地址和加密钱包文件哈希保持不变；尚未在你的设备验证。"],
  "zh-TW": ["Windows x64 0.6.10 無簽章測試預覽；系統重啟、使用者裝置、SmartScreen/Defender、已失敗資料恢復及真實交易未驗證。", "原始 0.6.10 安裝包在 Windows CI 中通過從 0.6.8 與 0.6.9 升級，地址與加密錢包檔案雜湊保持不變；尚未在你的裝置驗證。"],
  en: ["Unsigned Windows x64 0.6.10 Testnet preview; OS reboot, user device, SmartScreen/Defender, failed-profile recovery and live transfers remain unverified.", "The original 0.6.10 installer passed Windows CI upgrades from 0.6.8 and 0.6.9 with the address and encrypted vault hash preserved; it has not yet been verified on your device."],
  ja: ["Windows x64 0.6.10 は署名されていないテストネットプレビューです。OS 再起動、利用者の端末、SmartScreen/Defender、失敗したプロファイルの復旧、実際の送金は未検証です。", "元の 0.6.10 インストーラーは Windows CI で 0.6.8 と 0.6.9 からの更新に成功し、アドレスと暗号化されたウォレットファイルのハッシュは維持されました。お使いの端末ではまだ検証されていません。"],
  ko: ["Windows x64 0.6.10은 서명되지 않은 테스트넷 미리보기입니다. OS 재시작, 사용자 기기, SmartScreen/Defender, 실패한 프로필 복구 및 실제 전송은 검증되지 않았습니다.", "원본 0.6.10 설치 프로그램은 Windows CI에서 0.6.8 및 0.6.9에서의 업그레이드를 통과했으며 주소와 암호화된 지갑 파일 해시가 유지되었습니다. 사용자의 기기에서는 아직 검증되지 않았습니다."],
  es: ["Vista previa de Testnet 0.6.10 para Windows x64 sin firma. No se han verificado el reinicio del sistema, el dispositivo del usuario, SmartScreen/Defender, la recuperación de un perfil fallido ni las transferencias reales.", "El instalador original 0.6.10 superó en CI de Windows las actualizaciones desde 0.6.8 y 0.6.9, conservando la dirección y el hash del archivo cifrado de la cartera. Aún no se ha verificado en tu dispositivo."],
  fr: ["Aperçu Testnet 0.6.10 pour Windows x64 non signé. Le redémarrage du système, l'appareil de l'utilisateur, SmartScreen/Defender, la récupération d'un profil en échec et les transferts réels ne sont pas vérifiés.", "L'installateur original 0.6.10 a réussi en CI Windows les mises à niveau depuis 0.6.8 et 0.6.9, en conservant l'adresse et le hash du fichier chiffré du portefeuille. Il n'a pas encore été vérifié sur votre appareil."],
  de: ["Unsignierte Testnet-Vorschau 0.6.10 für Windows x64. Neustart des Betriebssystems, Nutzergerät, SmartScreen/Defender, Wiederherstellung eines fehlgeschlagenen Profils und echte Übertragungen sind nicht verifiziert.", "Der ursprüngliche Installer 0.6.10 bestand in der Windows-CI Upgrades von 0.6.8 und 0.6.9; Adresse und Hash der verschlüsselten Wallet-Datei blieben erhalten. Auf Ihrem Gerät wurde er noch nicht verifiziert."],
  pt: ["Prévia da Testnet 0.6.10 para Windows x64 sem assinatura. Reinicialização do sistema, dispositivo do usuário, SmartScreen/Defender, recuperação de perfil com falha e transferências reais não foram verificados.", "O instalador original 0.6.10 passou nos upgrades de 0.6.8 e 0.6.9 na CI do Windows, preservando o endereço e o hash do arquivo criptografado da carteira. Ainda não foi verificado no seu dispositivo."],
  ru: ["Неподписанная тестовая версия 0.6.10 для Windows x64. Перезагрузка ОС, устройство пользователя, SmartScreen/Defender, восстановление повреждённого профиля и реальные переводы не проверены.", "Исходный установщик 0.6.10 прошёл в Windows CI обновление с 0.6.8 и 0.6.9 с сохранением адреса и хеша зашифрованного файла кошелька. На вашем устройстве он ещё не проверен."],
  ar: ["معاينة شبكة الاختبار 0.6.10 لنظام Windows x64 غير موقّعة. لم يُتحقق من إعادة تشغيل النظام أو جهاز المستخدم أو SmartScreen/Defender أو استعادة ملف تعريف فاشل أو التحويلات الفعلية.", "اجتاز المثبّت الأصلي 0.6.10 ترقيات Windows CI من 0.6.8 و0.6.9 مع بقاء العنوان وتجزئة ملف المحفظة المشفّر دون تغيير. لم يُتحقق منه بعد على جهازك."],
  id: ["Pratinjau Testnet 0.6.10 untuk Windows x64 tanpa tanda tangan. Mulai ulang OS, perangkat pengguna, SmartScreen/Defender, pemulihan profil yang gagal, dan transfer nyata belum diverifikasi.", "Penginstal asli 0.6.10 lulus peningkatan dari 0.6.8 dan 0.6.9 di CI Windows dengan alamat dan hash berkas dompet terenkripsi tetap sama. Belum diverifikasi pada perangkat Anda."]
};
for (const [locale, copy] of Object.entries(WALLET_DOWNLOAD_COPY)) {
...
const BROWSER_014_COPY = {
  ...
  en: ["Unsigned, manually installed Chrome/Edge 0.1.4 Testnet preview; not in stores. Real user profiles, full public Finance connection, signing and transactions remain unverified.", "An isolated disposable Edge 153 profile passed empty-vault recovery, test-account creation and Finance static-origin reconnection; no real credentials were used."],
  ja: ["Chrome/Edge 0.1.4 は未署名の手動インストール用Testnetプレビューで、ストア未公開です。実利用者のプロファイル、公開Financeとの完全な接続、署名、取引は未検証です。", "隔離された一時的な Edge 153 プロファイルで空ウォレットの復旧、テストアカウント作成、Finance の静的同一オリジンへの再接続に成功しました。実際の認証情報は使っていません。"],
  ko: ["Chrome/Edge 0.1.4는 서명되지 않은 수동 설치 테스트넷 미리보기이며 스토어에 출시되지 않았습니다. 실제 사용자 프로필, 공개 Finance 전체 연결, 서명 및 거래는 검증되지 않았습니다.", "격리된 일회용 Edge 153 프로필에서 빈 지갑 복구, 테스트 계정 생성 및 Finance 정적 동일 출처 재연결을 통과했습니다. 실제 자격 증명은 사용하지 않았습니다."],
  es: ["Chrome/Edge 0.1.4 es una vista previa de Testnet sin firma, de instalación manual y no publicada en tiendas. No se han verificado perfiles reales, conexión pública completa con Finance, firmas ni transacciones.", "Un perfil aislado y desechable de Edge 153 superó la recuperación de cartera vacía, la creación de cuenta de prueba y la reconexión al origen estático de Finance; no se usaron credenciales reales."],
  fr: ["Chrome/Edge 0.1.4 est un aperçu Testnet non signé à installer manuellement, non publié en boutique. Les profils réels, la connexion publique complète à Finance, les signatures et les transactions ne sont pas vérifiés.", "Un profil Edge 153 isolé et jetable a réussi la récupération d'un portefeuille vide, la création d'un compte de test et la reconnexion à l'origine statique de Finance, sans identifiants réels."],
  de: ["Chrome/Edge 0.1.4 ist eine unsignierte, manuell installierte Testnet-Vorschau und nicht im Store. Echte Nutzerprofile, die vollständige öffentliche Finance-Verbindung, Signaturen und Transaktionen sind nicht verifiziert.", "Ein isoliertes, temporäres Edge-153-Profil bestand die Wiederherstellung einer leeren Wallet, die Erstellung eines Testkontos und die erneute Verbindung zur statischen Finance-Origin; echte Zugangsdaten wurden nicht verwendet."],
  pt: ["Chrome/Edge 0.1.4 é uma prévia da Testnet sem assinatura, de instalação manual e não publicada em lojas. Perfis reais, conexão pública completa com Finance, assinaturas e transações não foram verificados.", "Um perfil isolado e descartável do Edge 153 passou na recuperação de carteira vazia, criação de conta de teste e reconexão à origem estática do Finance; nenhuma credencial real foi usada."],
  ru: ["Chrome/Edge 0.1.4 — неподписанная тестовая версия для ручной установки, не опубликованная в магазинах. Реальные профили, полное подключение к публичному Finance, подписи и транзакции не проверены.", "В изолированном временном профиле Edge 153 прошли восстановление пустого кошелька, создание тестового аккаунта и повторное подключение к статическому источнику Finance; реальные учётные данные не использовались."],
  ar: ["Chrome/Edge 0.1.4 معاينة لشبكة الاختبار غير موقّعة تُثبَّت يدويًا وليست في المتاجر. لم تُتحقق ملفات المستخدمين الحقيقية أو وصلة Finance العامة الكاملة أو التوقيعات أو المعاملات.", "اجتاز ملف Edge 153 مؤقت ومعزول استعادة محفظة فارغة وإنشاء حساب اختبار وإعادة الاتصال بمصدر Finance الثابت؛ دون استخدام بيانات اعتماد حقيقية."],
  id: ["Chrome/Edge 0.1.4 adalah pratinjau Testnet tanpa tanda tangan yang dipasang manual dan tidak tersedia di toko. Profil pengguna nyata, koneksi publik Finance penuh, tanda tangan, dan transaksi belum diverifikasi.", "Profil Edge 153 sekali pakai yang terisolasi lulus pemulihan dompet kosong, pembuatan akun uji, dan koneksi ulang ke origin statis Finance; tanpa kredensial nyata."]
};
for (const [locale, copy] of Object.entries(WALLET_DOWNLOAD_COPY)) {
  [copy.browser014Boundary, copy.browser014Proof] = BROWSER_014_COPY[locale];
}
